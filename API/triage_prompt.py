...
def get_triage_prompt():
    ...
    return (
        ...
        f"\n\nImportant note: [book] should always be returned as its own action. It SHOULD NOT be returned in combination with other actions."
        ...
        f"\n   - Latest Arrival"
        f"\n\nExamples: "
        f"\n UserMessage: Update takeoff airport to IAH, return [update]."
        f"\n UserMessage: Update trip start date to Nov 22, return [update]."
        f"\n UserMessage: Change the number of carry on bags to 4, return [update]."
        f"\n UserMessage: Change the seat type to Business, return [update]."
        f"\n UserMessage: Filter out flights that stop at DCA, return [filter]."
        f"\n UserMessage: Only show flights with price less than 200, return [filter]."
        f"\n UserMessage: Exclude JetBlue and American flights, return [filter]."
        f"\n UserMessage: Sort data by lowest total price, return [sort]."
        f"\n UserMessage: Sort data by shortest duration, return [sort]."
        f"\n UserMessage: Show flights with earliest landing first, return [sort]."
        f"\n UserMessage: Show flights with latest takeoff first, return [sort]."
        f"\n UserMessage: Change the flight to be between IAH and JFK, update the start date to Nov 22, exclude American airlines flights and sort flights by total price, return [update, filter, sort]."
        f"\n UserMessage: Update the start date to Dec 22 and sort flights by earliest landing time, return [update, sort]."
        f"\n UserMessage: Book the 9:30am American flight for $118, return [book]."
        f"\n UserMessage: Book the 1st flight in the list, return [book]."
        f"\n UserMessage: I'll go with the nonstop JetBlue flight at 6:30am, return [book]."
        f"\n UserMessage: Finalize the Delta flight at 10:30am, return [book]."
    )